...
```{r}
#| label: fig-ga-toothgrowth-overview
#| fig-cap: "ToothGrowth数据集实验设计结构"
# 数据概览
...
```{r}
#| label: fig-ga-one-sample-ttest
#| fig-cap: "单样本t检验：样本均值与参考值比较"
# 设置理论参考值（可根据实际研究设定）
reference_value = 18
...
1.  **OJ 组不满足正态分布假设**（p = 0.024 < 0.05），而 VC 组满足。将不同处理组的数据合并分析可能会掩盖各组的特征。
2.  更好的分析策略是：
...
```{r}
#| label: fig-ga-wilcoxon-histogram
#| fig-cap: "牙齿生长长度分布直方图"
library(ggplot2)
...
```{r}
#| label: fig-ga-independent-ttest-boxplot
#| fig-cap: "两种补充剂对牙齿生长影响的箱线图"
library(tidyverse)
...
```{r}
#| label: ga-independent-ttest-assumptions
# 正态性检验
shapiro_OJ = shapiro.test(
  ToothGrowth$len[ToothGrowth$supp == "OJ"])
shapiro_VC = shapiro.test(
...
```{r}
#| label: ga-independent-ttest-result
# 进行 t 检验
...
```{r}
#| label: fig-ga-wilcoxon-ranksum-plot
#| fig-cap: "OJ与VC补充剂对牙齿生长的影响（Wilcoxon检验）"
# 加载必要的包
library(ggpubr)
...
ggplot(ToothGrowth,
  ...
  stat_compare_means(method = "wilcox.test",
                     label = "p.format") +
  labs(
    title = "OJ vs. VC 补充剂对牙齿生长的影响（Wilcoxon 检验）",
    x = "补充剂类型",
    y = "牙齿生长长度") +
  theme_bw() +
  guides(color = "none")
...
1.  **`ggplot() + geom_boxplot()`**：使用标准 ggplot2 语法绘制分组箱线图，`color = supp` 区分类别。\
2.  **`geom_jitter()`**：添加原始数据点，`width = 0.2` 控制水平抖动，避免点重叠。\
3.  **`stat_compare_means(method = "wilcox.test", label = "p.format")`**：
...
```{r}
#| label: ga-wilcoxon-method-choice
# 正态性检验
shapiro_OJ = shapiro.test(
  ToothGrowth$len[ToothGrowth$supp == "OJ"])
shapiro_VC = shapiro.test(
...
```{r}
#| label: fig-ga-paired-ttest-plot
#| fig-cap: "两种药物对睡眠时间的影响（配对t检验）"
# 加载必要的包
library(ggpubr)
...
-   **箱线图** 直观显示 **睡眠时间的分布**，灰色连线表示相同受试者在不同药物下的数据（通过 `geom_line(aes(group = ID))` 实现配对连线）。
...
```{r}
#| label: fig-ga-wilcoxon-signed-rank-plot
#| fig-cap: "两种药物对睡眠时间的影响（Wilcoxon符号秩检验）"
# 加载必要的包
library(ggpubr)
...
ggplot(sleep,
  ...
  stat_compare_means(method = "wilcox.test",
                     ...
                     label = "p.format") +
  labs(
    title = "两种药物对睡眠时间的影响（Wilcoxon 符号秩检验）",
    x = "药物组",
    y = "额外睡眠时间") +
  theme_bw() +
  guides(color = "none")
...
```{r}
#| label: fig-ga-anova-vc
#| fig-cap: "VC处理下不同剂量对牙齿生长的影响（ANOVA）"
# 加载必要的包
library(ggpubr)
...
ggplot(vc_data,
  ...
  labs(
    title = "VC 处理下不同剂量对牙齿生长的影响（ANOVA）",
    x = "剂量",
    y = "牙齿长度",
    color = "剂量") +
  theme_bw()
...
```{r}
#| label: fig-ga-anova-tukey
#| fig-cap: "Tukey HSD多重比较结果"
# 进行Tukey HSD多重比较
...
```{r}
#| label: fig-ga-kruskal-wallis
#| fig-cap: "VC处理下不同剂量对牙齿生长的影响（Kruskal-Wallis）"
...
ggplot(vc_data,
  ...
  labs(
    title = "VC 处理下不同剂量对牙齿生长的影响（Kruskal-Wallis）",
    x = "剂量",
    y = "牙齿长度",
    color = "剂量") +
  theme_bw()
...
```{r}
#| label: fig-ga-kruskal-dunn
#| fig-cap: "VC剂量组间的Dunn多重比较"
# 使用Dunn检验进行多重比较
library(dunn.test)
...
```{r}
#| label: ga-kruskal-method-choice
# 检查各组的正态性和方差齐性
...
```{r}
#| label: fig-ga-pearson-scatter
#| fig-cap: "Pearson相关性分析：马力与燃油效率"
...
ggplot(mtcars, aes(x = hp, y = mpg)) +
  ...
  labs(
    title = "Pearson 相关性分析：马力 vs. 燃油效率",
    x = "马力 (hp)",
    y = "燃油效率 (mpg)") +
  theme_bw()
```
...
```{r}
#| label: fig-ga-pearson-assumptions
#| fig-cap: "马力与燃油效率的LOESS曲线检查"
# 正态性检验
...
```{r}
#| label: fig-ga-spearman-scatter
#| fig-cap: "Spearman相关性分析：马力与燃油效率"
...
```{r}
#| label: fig-ga-cor-comparison
#| fig-cap: "Pearson与Spearman相关分析方法比较"
# 创建比较数据框
...
```{r}
#| label: fig-ga-mantel-plot
#| fig-cap: "Mantel相关性分析散点图"
# 提取距离矩阵中的非对角元素
